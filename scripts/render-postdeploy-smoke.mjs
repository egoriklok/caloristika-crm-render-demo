...
results.push(await check("/demo", { includes: "CRM для B2B-продаж готовой еды: от первого интереса до повторного заказа", excludes: "Нужен ключ доступа" }))
results.push(await check("/crm?tab=pipeline", { includes: "Загрузка единой базы", excludes: "Нужен ключ доступа" }))
