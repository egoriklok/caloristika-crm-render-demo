...
const translationPairs: Array<[string, string]> = [
  ...
  [
    "Когда поставщик готовой еды запускает B2B-продажи, команде нужно быстро понять, какие компании подходят, что предложить каждому сегменту, как провести дегустацию, принять первый заказ и закрепить повтор. Demo собирает этот путь в одном рабочем месте: локальные лиды из 2GIS, ФНС/DaData enrichment, маршрут от производства, сегменты, SKU-матрицы, клиентский каталог, Telegram Mini App, заказы и AI-задачи менеджеру.",
    "When a ready-meal supplier launches B2B sales, the team needs to quickly understand which companies fit, what to offer each segment, how to run a tasting, take the first order and secure the repeat. The demo keeps that path in one workspace: local leads from 2GIS, FNS/DaData enrichment, route from production, segments, SKU matrices, client catalog, Telegram Mini App, orders and AI tasks for the manager."
  ],
  ...
  ["источник лида, адрес, маршрут, сегмент, товарную матрицу, каталог, заказ и задачу менеджеру.", "lead source, address, route, segment, product matrix, catalog, order and manager task."],
  ...
  ["Откуда берется база", "Where the base comes from"],
  ["База", "Base"],
  [
    "Локальные B2B-лиды строятся от производства и попадают в CRM только после проверки",
    "Local B2B leads are built from the production point and enter CRM only after verification"
  ],
  [
    "Слой Lunch Up CRM задокументирован как управляемый pipeline, а не ручная таблица: сначала поиск точек вокруг производства через 2GIS, затем ФНС/DaData enrichment, затем dry-run и подтвержденный импорт. Яндекс.Карты и открытые карты используются для ручной проверки маршрута и здравого смысла логистики.",
    "The Lunch Up CRM layer is documented as a managed pipeline, not a manual spreadsheet: first point search around production through 2GIS, then FNS/DaData enrichment, then dry-run and confirmed import. Yandex Maps and open maps are used for manual route and logistics sense checks."
  ],
  ["По документации Lunch Up CRM поиск начинается как dry-run: кандидат, адрес, публичные контакты, карточка источника и сегмент попадают в предварительный список.", "According to the Lunch Up CRM documentation, search starts as a dry-run: candidate, address, public contacts, source card and segment enter the preview list."],
  ["После проверки карточка обогащается ИНН, реквизитами, headcount evidence и статусом юрлица, чтобы менеджер видел не только название компании.", "After review, the card is enriched with tax ID, company details, headcount evidence and legal-entity status, so the manager sees more than a company name."],
  ["Запись в CRM делается только после confirm_import. Источники сохраняются в company_enrichment_sources, поэтому можно проверить, откуда взялась запись.", "The CRM record is created only after confirm_import. Sources are stored in company_enrichment_sources, so the origin of the record can be checked."],
  ["Для локальной поставки важна логистика: минуты от производства, район, карта и ручная проверка маршрута через открытые карты или Яндекс.Карты.", "For local delivery, logistics matters: minutes from production, district, map and a manual route check through open maps or Yandex Maps."],
  ["2GIS поиск лидов", "2GIS lead search"],
  ["ФНС/DaData-обогащение", "FNS/DaData enrichment"],
  ["Подтвержденный импорт", "Confirmed import"],
  ["ФНС/DaData enrichment", "FNS/DaData enrichment"],
  ["Маршрут от производства", "Route from production"],
  ["Открыть базу в CRM", "Open base in CRM"],
  ...
  ["Архитектурная витрина: как повторять запуск под новую компанию, подключать агентов, 2GIS/ФНС источники, Telegram и внешние API.", "Architecture showcase: how to repeat the launch for a new company and connect agents, 2GIS/FNS sources, Telegram and external APIs."],
  ...
  ["CRM собирает локацию, сегмент, 2GIS/ФНС источник, маршрут от производства и следующий шаг для менеджера.", "CRM collects location, segment, 2GIS/FNS source, route from production and the manager's next step."],
  ...
  ["100-150 B2B-компаний с 2GIS/ФНС источниками", "100-150 B2B companies with 2GIS/FNS sources"],
