...
Живой Telegram-канал на Render:

```text
https://t.me/b2b_food_crm_demo_bot
https://caloristika-crm-demo.onrender.com/miniapp
https://caloristika-crm-demo.onrender.com/crm?tab=dialogs
```

Бот уже создан через BotFather, webhook подключен к Render, Mini App открывает
каталог, а неслужебные клиентские сообщения создают черновики для менеджера.
Операционный runbook: `docs/TELEGRAM_CHANNEL_RUNBOOK.md`.
...
### Публичная ссылка для клиентов и портфолио

Для внешней отправки используйте публичный вход в CRM без ключа:

```text
https://caloristika-crm-demo.onrender.com/crm
```

`/crm` открывает Caloristika dashboard для первого просмотра: воронка, единая
база, компании, контакты, каталог, заказы и AI-задачи. `/demo` объясняет JTBD
продукта, офферы и сценарий показа:

```text
https://caloristika-crm-demo.onrender.com/demo
```

Публичные страницы не раскрывают CRM key, API-токены и внутренние integration
routes. Защищенный операторский вход с `?key=...` отправлять только выбранному
человеку. Caloristika используется как пример компании для демонстрации работы
CRM, а не как подтвержденный партнер. Готовые тексты для клиента и портфолио:
...
3. Не вставлять токен в чат, README, CRM-поля или Git.
4. Выполнить `npm run telegram:set-token`.
5. Скрипт скрыто спросит BotFather token, сохранит его в `.env.local`, обновит Render env, запустит deploy и выполнит `npm run telegram:setup`.
6. Запустить `npm run telegram:check -- --json`, чтобы увидеть готовность, Mini App entrypoints `/order`, `/cart`, `/cabinet`, `/orders`, `/whoami` и следующие действия без вывода токенов.

Живой demo-бот уже подключен: `https://t.me/b2b_food_crm_demo_bot`.
Подробный runbook: `docs/TELEGRAM_CHANNEL_RUNBOOK.md`.
...
Чтобы включить уведомления менеджеру в новом проекте, после настройки webhook откройте бота в Telegram, отправьте `/whoami`, скопируйте полученный `Telegram chat id` в `TELEGRAM_MANAGER_CHAT_ID` внутри `.env.local` и выполните `npm run render:env:update -- --public-url https://<render-service>.onrender.com --deploy`. В текущем Render demo manager chat id уже настроен.
...
set TELEGRAM_BOT_TOKEN=<store via npm run telegram:set-token>
set TELEGRAM_WEBHOOK_SECRET=<generated by telegram:set-token/render updater>
set TELEGRAM_MANAGER_CHAT_ID=manager-chat-id
set TELEGRAM_BOT_DISPLAY_NAME=B2B Food CRM Demo
set TELEGRAM_BOT_DESCRIPTION=Демо CRM для B2B-продаж готовой еды: каталог, корзина, заявки и AI-помощник менеджера.
set TELEGRAM_BOT_SHORT_DESCRIPTION=Каталог, корзина и B2B-заявки через Telegram Mini App.
set TELEGRAM_MENU_BUTTON_TEXT=Открыть каталог
set TELEGRAM_MINIAPP_SHORT_NAME=
set PUBLIC_BASE_URL=https://caloristika-crm-demo.onrender.com
...
- `TELEGRAM_BOT_TOKEN` нужен для проверки Mini App `initData` и отправки кнопки Mini App из webhook. Для оператора безопаснее записывать его через `npm run telegram:set-token`, а не через shell history.
