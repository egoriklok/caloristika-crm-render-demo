...
Для внешней отправки используйте публичный overview без ключа:

```text
https://caloristika-crm-demo.onrender.com/demo
```

Он показывает, что делает demo CRM, дает безопасные входы в клиентский каталог
и Mini App, но не раскрывает CRM key, API-токены и внутренние integration
routes. Полный dashboard этой demo CRM отправлять только отдельной ссылкой с
`?key=...` выбранному человеку. Caloristika используется как пример компании для
демонстрации работы CRM, а не как подтвержденный партнер. Готовые тексты для
клиента и портфолио: `docs/PORTFOLIO_DEMO_SHARE_KIT.md`.
